...
const steps=[];
function run(name,command,args){const startedAt=new Date().toISOString(),result=spawnSync(command,args,{encoding:"utf8",stdio:["ignore","pipe","pipe"],env:{...process.env,AIONEX_CONTENT_PROVIDER:process.env.AIONEX_CONTENT_PROVIDER||"ollama",AIONEX_REQUIRE_AI_CONTENT:"true"}}),entry={name,startedAt,finishedAt:new Date().toISOString(),exitCode:result.status,stdout:String(result.stdout||"").slice(-12_000),stderr:String(result.stderr||"").slice(-4_000)};steps.push(entry);if(result.status!==0)throw new Error(`${name} failed: ${(entry.stderr||entry.stdout).slice(-1200)}`);return entry;}
let ok=false,error;
try{run("narrated_pipeline","npm.cmd",["run","pipeline:dry-run","--",`--day=${slot}`]);run("telegram_publish",process.execPath,["--env-file-if-exists=.env.local",resolve("scripts","publish-daily-telegram-video.mjs"),`--day=${slot}`]);run("youtube_publish",process.execPath,["--env-file-if-exists=.env.local",resolve("scripts","publish-daily-youtube-video.mjs"),`--day=${slot}`]);ok=true;}catch(caught){error=caught instanceof Error?caught.message:String(caught);}
