...
export type VideoValidation={passed:boolean;productionReady:boolean;errors:string[];width:number;height:number;duration:number;videoCodec:string;audioCodec:string;fileSize:number};

export function validateVideoProbe(probe:VideoProbe,fileExists:boolean):VideoValidation{
  const video=probe.streams?.find(stream=>stream.codec_type==="video"),audio=probe.streams?.find(stream=>stream.codec_type==="audio"),width=Number(video?.width||0),height=Number(video?.height||0),duration=Number(probe.format?.duration||0),fileSize=Number(probe.format?.size||0),videoCodec=String(video?.codec_name||""),audioCodec=String(audio?.codec_name||""),errors:string[]=[];
  ...
  return{passed:errors.length===0,productionReady:errors.length===0,errors,width,height,duration,videoCodec,audioCodec,fileSize};
}
