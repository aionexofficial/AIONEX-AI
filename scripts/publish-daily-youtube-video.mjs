...
async function setThumbnail(accessToken,uploadId,videoId){const response=await fetch(`https://www.googleapis.com/upload/youtube/v3/thumbnails/set?videoId=${encodeURIComponent(videoId)}&uploadType=media`,{method:"POST",headers:{Authorization:`Bearer ${accessToken}`,"Content-Type":"image/jpeg"},body:thumbnail}),payload=await response.json().catch(()=>({}));if(!response.ok){const detail=payload.error?.errors?.map(error=>error.reason).filter(Boolean).join(", ")||payload.error?.message||response.statusText,message=`YouTube thumbnail upload failed (${response.status}): ${detail}`;await sql`UPDATE youtube_uploads SET status='failed',video_id=${videoId},last_error=${message},updated_at=NOW() WHERE id=${uploadId}::uuid`;const failure=new Error(message);failure.videoId=videoId;throw failure;}await sql`UPDATE youtube_uploads SET video_id=${videoId},status='published',last_error=NULL,published_at=COALESCE(published_at,NOW()),updated_at=NOW() WHERE id=${uploadId}::uuid`;}
async function upload(accessToken,variant,title,description,tags){
  ...
  if(existing[0]?.video_id){const videoId=String(existing[0].video_id);await setThumbnail(accessToken,String(existing[0].id),videoId);return{variant,videoId,skipped:false,resumed:true};}
  ...
  await setThumbnail(accessToken,uploadId,String(body.id));
...
for(const publication of [{variant:"short",title:`${baseTitle} #Shorts`,description:`${description}\n\n#Shorts`},{variant:"video",title:baseTitle,description}])try{results.push(await upload(accessToken,publication.variant,publication.title,publication.description,tags));}catch(error){errors.push({variant:publication.variant,videoId:error?.videoId||null,message:error instanceof Error?error.message:String(error)});}
const report={ok:errors.length===0,slot,visibility:"public",thumbnail:errors.length===0,results,errors,x:false,tiktok:false,publishedAt:new Date().toISOString()};
await writeFile(reportPath,JSON.stringify(report,null,2));
console.log(JSON.stringify(report,null,2));
if(errors.length)throw new Error(errors.map(error=>`${error.variant}: ${error.message}`).join(" | "));
