...
const day=process.argv.find(argument=>argument.startsWith("--day="))?.slice(6)||new Date().toISOString().slice(0,13).replace(":","-");
...
const renderScale=Math.max(.25,Math.min(1,Number(process.env.REMOTION_RENDER_SCALE)||.5));
...
if(!contentArgument&&!existsSync(generatedContentPath))execute(process.execPath,["--conditions=react-server","--experimental-strip-types",resolve("scripts","generate-local-video-content.ts"),`--day=${day}`,`--output=${generatedContentPath}`]);
...
else{const serveUrl=await bundle({entryPoint:resolve("remotion","index.ts")}),composition=await selectComposition({serveUrl,id:"AionexDaily",inputProps:generated.content});await renderMedia({serveUrl,composition,codec:"h264",pixelFormat:"yuv420p",outputLocation:sourceVideo,inputProps:generated.content,overwrite:true,concurrency:1,scale:renderScale,disallowParallelEncoding:true,offthreadVideoThreads:1});}
...
execute("ffmpeg",["-y","-i",sourceVideo,"-i",narrationPath,"-filter_complex",`${filter};[0:v]scale=1080:1920:flags=lanczos[video]`,"-map","[video]","-map","[narration]","-t","30","-c:v","libx264","-preset","medium","-crf","20","-pix_fmt","yuv420p","-c:a","aac","-b:a","128k","-movflags","+faststart",validatedVideo]);
