...
import { DailyTaskManager } from "@/components/admin/daily-task-manager";
...
export default async function AdminRewardsPage(){
 ...
 return <><RewardsManager initialTasks={tasks.map(r=>({id:String(r.id),category:r.category as TaskCategory,task_group:r.task_group as TaskGroup,title:String(r.title),description:String(r.description),icon:String(r.icon),reward_axp:Number(r.reward_axp),reward_xp:Number(r.reward_xp),difficulty:String(r.difficulty),enabled:Boolean(r.enabled),repeat_mode:String(r.repeat_mode),cooldown_hours:r.cooldown_hours===null?null:Number(r.cooldown_hours),verification_mode:String(r.verification_mode),verification_config:(r.verification_config||{}) as Record<string,unknown>,task_url:r.task_url?String(r.task_url):null,completion_count:Number(r.completion_count||0)}))} initialUsers={users.map(r=>({id:String(r.id),displayName:String(r.display_name),axp:Number(r.axp_balance),xp:Number(r.xp),level:Number(r.level),status:String(r.status),riskScore:Number(r.risk_score),referrals:Number(r.referrals)}))} initialSettings={Object.fromEntries(settings.map(r=>[String(r.key),Number(r.value)]))} stats={stats}/><div className="mx-auto -mt-8 max-w-7xl px-5 pb-8 sm:px-8"><DailyTaskManager/></div></>;
}
