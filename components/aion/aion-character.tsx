...
export function AionCharacter({ stage, color = "cyan", reaction = "idle", onTap, disabled = false, label = "Tap AION" }: { stage: AionStageKey; color?: string; reaction?: AionReaction; onTap?: () => void; disabled?: boolean; label?: string }) {
  const reduced = useReducedMotion(), scale = stageScale[stage];
  ...
  return <motion.button type="button" aria-label={label} disabled={disabled} onPointerDown={onTap}
    ...
    transition={reaction === "idle" || reaction === "tired" ? { duration: 3.2, repeat: Infinity, ease: "easeInOut" } : { duration: .32 }}
    ...
    {stage === "prime" && <span className="absolute -top-1 text-[9px] font-black uppercase tracking-[.32em] text-amber-200">Prime</span>}
  </motion.button>;
}
