...
const inserted=await sql`INSERT INTO generated_content(content_hash,topic,format,title,body,metadata) VALUES(${contentHash},'AIONEX scheduled narrated video','aionex_local_video',${String(metadata.content?.title||"AIONEX Daily")},${String(metadata.content?.script||"")},${JSON.stringify({scheduledAutomation:true,destinations:["telegram","youtube_short","youtube_video"],narrated:true,slot:day,videoSha256:createHash("sha256").update(video).digest("hex")})}::jsonb) ON CONFLICT(content_hash) DO NOTHING RETURNING id`;
