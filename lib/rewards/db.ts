...
import { taskClaimKey } from "../aion/rules";
...
export async function findOrCreateIdentity(provider: "wallet" | "telegram", providerUserId: string, metadata: Record<string, unknown>, currentUserId?: string | null) {
  const db = sql();
  const generatedUserId=randomUUID(),fallbackDisplay=String(metadata.firstName||metadata.username||metadata.address||"AIONEX Explorer").replace(/[<>\u0000-\u001f]/g,"").trim().slice(0,80)||"AIONEX Explorer";
  const results=await db.transaction(tx=>[
    tx`SELECT pg_advisory_xact_lock(hashtextextended(${`${provider}:${providerUserId}`},0))`,
    tx`INSERT INTO reward_users(id,display_name,referral_code) SELECT ${generatedUserId}::uuid,${fallbackDisplay},${referralCode()} WHERE NOT EXISTS(SELECT 1 FROM reward_identities WHERE provider=${provider} AND provider_user_id=${providerUserId}) AND (${currentUserId||null}::uuid IS NULL OR NOT EXISTS(SELECT 1 FROM reward_users WHERE id=${currentUserId||null}::uuid AND status='active')) ON CONFLICT(id) DO NOTHING`,
    tx`INSERT INTO reward_identities(user_id,provider,provider_user_id,metadata) SELECT COALESCE((SELECT user_id FROM reward_identities WHERE provider=${provider} AND provider_user_id=${providerUserId}),(SELECT id FROM reward_users WHERE id=${currentUserId||null}::uuid AND status='active'),${generatedUserId}::uuid),${provider},${providerUserId},${JSON.stringify(metadata)}::jsonb ON CONFLICT(provider,provider_user_id) DO UPDATE SET metadata=EXCLUDED.metadata,verified_at=NOW() RETURNING user_id`,
  ]);
  const linked=results[2];if(!linked[0])throw new Error("Identity provisioning failed.");const userId=String(linked[0].user_id);
  ...
  await db`INSERT INTO aion_character_profiles(user_id,current_energy,max_energy,energy_regen_amount,energy_regen_interval_seconds,tap_power,critical_chance_bps,critical_multiplier_bps,economy_version) SELECT u.id,COALESCE((SELECT value FROM reward_settings WHERE key='aion_default_max_energy'),1000),COALESCE((SELECT value FROM reward_settings WHERE key='aion_default_max_energy'),1000),COALESCE((SELECT value FROM reward_settings WHERE key='aion_energy_regen_amount'),1),COALESCE((SELECT value FROM reward_settings WHERE key='aion_energy_regen_interval_seconds'),6),COALESCE((SELECT value FROM reward_settings WHERE key='aion_base_tap_power'),1),COALESCE((SELECT value FROM reward_settings WHERE key='aion_critical_chance_bps'),500),COALESCE((SELECT value FROM reward_settings WHERE key='aion_critical_multiplier_bps'),20000),COALESCE((SELECT value FROM reward_settings WHERE key='aion_economy_version'),1) FROM reward_users u WHERE u.id=${userId}::uuid ON CONFLICT(user_id) DO NOTHING`;
  return userId;
}
...
export async function listTasks(userId?: string | null): Promise<RewardTask[]> {
  const rows = userId
    ? await sql()`SELECT t.*,c.status AS claim_status,c.verified_at AS completed_at,(SELECT COUNT(*)::int FROM reward_task_claims x WHERE x.task_id=t.id AND x.status='verified') AS completion_count FROM reward_tasks t LEFT JOIN LATERAL (SELECT status,verified_at FROM reward_task_claims WHERE task_id=t.id AND user_id=${userId}::uuid AND (t.repeat_mode='once' OR (t.repeat_mode='daily' AND claim_key=CURRENT_DATE::text) OR (t.repeat_mode='cooldown' AND claim_key='period:'||(FLOOR(EXTRACT(EPOCH FROM NOW())/(GREATEST(COALESCE(t.cooldown_hours,24),1)*3600)))::bigint::text)) ORDER BY created_at DESC LIMIT 1)c ON TRUE WHERE t.enabled=TRUE AND (t.starts_at IS NULL OR t.starts_at<=NOW()) AND (t.ends_at IS NULL OR t.ends_at>NOW()) ORDER BY t.task_group,t.sort_order,t.created_at`
...
export async function mine(userId: string, rewardAxp?: number) {
  ...
  const xpPerLevel = await rewardSetting("aion_xp_per_level", 500);
  ...
  UPDATE reward_users u SET last_mined_at=NOW(),updated_at=NOW(),axp_balance=u.axp_balance+l.amount,lifetime_axp=u.lifetime_axp+l.amount,xp=u.xp+l.xp_awarded,level=1+FLOOR((u.xp+l.xp_awarded)/${xpPerLevel}::numeric)::int
...
export async function dailyCheckin(userId: string, baseAxp?: number) {
  ...
  const [bonus7,bonus30,xp7,xp30]=await Promise.all([rewardSetting("daily_streak_7_bonus_axp",100),rewardSetting("daily_streak_30_bonus_axp",750),rewardSetting("daily_streak_7_bonus_xp",50),rewardSetting("daily_streak_30_bonus_xp",300)]);
  const xpPerLevel=await rewardSetting("aion_xp_per_level",500);
  const rows = await sql()`WITH eligible AS (
    ...
    SELECT id,${baseAxp}+(LEAST(next_streak,7)-1)*5+CASE WHEN next_streak%30=0 THEN ${bonus30} WHEN next_streak%7=0 THEN ${bonus7} ELSE 0 END,${xp}+CASE WHEN next_streak%30=0 THEN ${xp30} WHEN next_streak%7=0 THEN ${xp7} ELSE 0 END,'daily_login','login:'||id||':'||CURRENT_DATE,jsonb_build_object('streak',next_streak,'milestone',CASE WHEN next_streak%30=0 THEN 30 WHEN next_streak%7=0 THEN 7 ELSE NULL END) FROM eligible
    ON CONFLICT (idempotency_key) DO NOTHING RETURNING user_id,amount,xp_awarded
  )
  UPDATE reward_users u SET login_streak=e.next_streak,last_checkin_date=CURRENT_DATE,updated_at=NOW(),axp_balance=u.axp_balance+l.amount,lifetime_axp=u.lifetime_axp+l.amount,xp=u.xp+l.xp_awarded,level=1+FLOOR((u.xp+l.xp_awarded)/${xpPerLevel}::numeric)::int
...
export async function stopMiningSession(userId: string) {
  ...
  const baseXp = await rewardSetting("mining_xp", 25), xpPerLevel=await rewardSetting("aion_xp_per_level",500), axp = Math.max(1, Math.floor(base*elapsed/total)), xp = Math.max(1, Math.floor(baseXp*elapsed/total));
...
export async function applyReferral(userId: string, code: string, deviceHash?: string | null, ipHash?: string | null) {
  ...
  const referrerXp = await rewardSetting("referrer_xp", 50), referredXp=await rewardSetting("referred_xp",25),xpPerLevel=await rewardSetting("aion_xp_per_level",500);
  const rows = await sql()`WITH referral AS (
    ...
    SELECT awards.user_id,awards.amount,awards.xp,'referral',awards.reference_id,awards.idempotency_key
    FROM referral r CROSS JOIN LATERAL (VALUES
      (r.referrer_id,${referrerAxp}::bigint,${referrerXp}::int,r.user_id,'referrer:'||r.user_id),
      (r.user_id,${referredAxp}::bigint,${referredXp}::int,r.referrer_id,'referred:'||r.user_id)
    ) AS awards(user_id,amount,xp,reference_id,idempotency_key)
    ON CONFLICT(idempotency_key) DO NOTHING
    RETURNING user_id,amount,xp_awarded,reference_id,idempotency_key
  ), economy AS (
    INSERT INTO aion_economy_transactions(user_id,transaction_type,axp_delta,xp_delta,reference_type,reference_id,idempotency_key,metadata)
    SELECT user_id,'referral_reward',amount,xp_awarded,'referral',reference_id,'aion:'||idempotency_key,jsonb_build_object('referralCode',${code}::text) FROM ledger
    ON CONFLICT(idempotency_key) DO NOTHING RETURNING id
  ), referral_event AS (
    INSERT INTO aion_referral_events(referred_user_id,referrer_user_id,device_hash,ip_hash,referrer_axp,referred_axp,referrer_xp,referred_xp)
    SELECT user_id,referrer_id,${deviceHash||null},${ipHash||null},${referrerAxp},${referredAxp},${referrerXp},${referredXp} FROM referral ON CONFLICT(referred_user_id) DO NOTHING RETURNING id
  ), totals AS (
    SELECT user_id,SUM(amount) AS amount FROM ledger GROUP BY user_id
  )
  UPDATE reward_users u SET referred_by=CASE WHEN u.id=r.user_id THEN r.referrer_id ELSE u.referred_by END,axp_balance=u.axp_balance+t.amount,lifetime_axp=u.lifetime_axp+GREATEST(t.amount,0),xp=u.xp+t.xp,level=1+FLOOR((u.xp+t.xp)/${xpPerLevel}::numeric)::int,updated_at=NOW()
  FROM (SELECT user_id,SUM(amount) amount,SUM(xp_awarded) xp FROM ledger GROUP BY user_id)t CROSS JOIN referral r WHERE u.id=t.user_id RETURNING u.id`;
...
export async function claimTask(userId: string, taskId: string, evidence: Record<string, unknown>, externallyVerified=false) {
  ...
  const claimKey = taskClaimKey(task.repeat_mode, new Date(), cooldownHours);
...
export async function systemTaskEligible(userId:string,category:string,config:Record<string,unknown>={}){
  const start=typeof config.periodStart==="string"?config.periodStart:null,end=typeof config.periodEnd==="string"?config.periodEnd:null;
  if(category==="daily_login")return Boolean((await sql()`SELECT 1 FROM reward_point_ledger WHERE user_id=${userId}::uuid AND reason='daily_login' AND created_at>=COALESCE(${start}::timestamptz,CURRENT_DATE) AND (${end}::timestamptz IS NULL OR created_at<${end}::timestamptz) LIMIT 1`)[0]);
  if(category==="daily_mining"||category==="tap_milestone"){const required=Math.max(1,Math.min(100000000,Number(config.requiredTaps||1)));if(start)return Boolean((await sql()`SELECT 1 FROM aion_tap_batches WHERE user_id=${userId}::uuid AND server_received_at>=${start}::timestamptz AND (${end}::timestamptz IS NULL OR server_received_at<${end}::timestamptz) GROUP BY user_id HAVING SUM(accepted_taps)>=${required}`)[0]);if(category==="tap_milestone")return Boolean((await sql()`SELECT 1 FROM aion_character_profiles WHERE user_id=${userId}::uuid AND total_taps>=${required} LIMIT 1`)[0]);return Boolean((await sql()`SELECT 1 FROM aion_tap_batches WHERE user_id=${userId}::uuid AND server_received_at>=CURRENT_DATE GROUP BY user_id HAVING SUM(accepted_taps)>=${required}`)[0]);}
  if(category==="referral_invite"){const required=Math.max(1,Math.min(10000,Number(config.requiredReferrals||1)));if(start)return Boolean((await sql()`SELECT 1 FROM aion_referral_events WHERE referrer_user_id=${userId}::uuid AND created_at>=${start}::timestamptz AND (${end}::timestamptz IS NULL OR created_at<${end}::timestamptz) GROUP BY referrer_user_id HAVING COUNT(*)>=${required}`)[0]);return Boolean((await sql()`SELECT 1 FROM aion_referral_events WHERE referrer_user_id=${userId}::uuid AND created_at>=CURRENT_DATE GROUP BY referrer_user_id HAVING COUNT(*)>=${required}`)[0]);}
  if(category==="ai_chat"){const required=Math.max(2,Math.min(100,Number(config.requiredMessages||2)));return Boolean((await sql()`SELECT 1 FROM aion_ai_conversations c WHERE c.user_id=${userId}::uuid AND (SELECT COUNT(*) FROM aion_ai_messages m WHERE m.conversation_id=c.id AND m.created_at>=COALESCE(${start}::timestamptz,'epoch'::timestamptz) AND (${end}::timestamptz IS NULL OR m.created_at<${end}::timestamptz))>=${required} LIMIT 1`)[0]);}
  if(category==="referral_milestone"){const required=Math.max(1,Math.min(10000,Number(config.requiredReferrals||1)));return Boolean((await sql()`SELECT 1 FROM reward_users WHERE referred_by=${userId}::uuid GROUP BY referred_by HAVING COUNT(*)>=${required}`)[0]);}
  if(category==="achievement_milestone"){const required=Math.max(1,Math.min(10000,Number(config.requiredAchievements||1)));return Boolean((await sql()`SELECT 1 FROM reward_user_badges WHERE user_id=${userId}::uuid GROUP BY user_id HAVING COUNT(*)>=${required}`)[0]);}
  return false;
}
...
export async function awardPoints(userId: string, amount: number, reason: "task" | "referral" | "achievement" | "admin_adjustment", referenceId: string, idempotencyKey: string,xpOverride?:number) {
  ...
  const xpPerLevel=await rewardSetting("aion_xp_per_level",500);
  return sql()`WITH ledger AS (INSERT INTO reward_point_ledger(user_id,amount,xp_awarded,reason,reference_id,idempotency_key) VALUES(${userId}::uuid,${amount},${xp},${reason},${referenceId}::uuid,${idempotencyKey}) ON CONFLICT(idempotency_key) DO NOTHING RETURNING user_id,amount,xp_awarded,reference_id), task_reward AS (SELECT l.user_id,l.reference_id,t.id AS task_id,t.daily_period_id,t.reward_energy,t.reward_chest_progress,t.reward_streak_progress FROM ledger l JOIN reward_task_claims c ON c.id=l.reference_id JOIN reward_tasks t ON t.id=c.task_id WHERE ${reason}='task'), energy_update AS (UPDATE aion_character_profiles p SET current_energy=LEAST(p.max_energy,p.current_energy+r.reward_energy),updated_at=NOW() FROM task_reward r WHERE p.user_id=r.user_id RETURNING p.user_id), progress_update AS (UPDATE daily_task_user_progress p SET progress=1,chest_progress=p.chest_progress+r.reward_chest_progress,streak_progress=p.streak_progress+r.reward_streak_progress,completed_at=COALESCE(p.completed_at,NOW()),claimed_at=COALESCE(p.claimed_at,NOW()),updated_at=NOW() FROM task_reward r WHERE p.period_id=r.daily_period_id AND p.user_id=r.user_id AND p.task_id=r.task_id RETURNING p.user_id) UPDATE reward_users u SET axp_balance=u.axp_balance+l.amount,lifetime_axp=u.lifetime_axp+GREATEST(l.amount,0),xp=u.xp+l.xp_awarded,level=1+FLOOR((u.xp+l.xp_awarded)/${xpPerLevel}::numeric)::int,updated_at=NOW() FROM ledger l WHERE u.id=l.user_id RETURNING u.*`;
}
...
export async function evaluateBadges(userId: string) { await sql()`SELECT aion_award_progress_achievements(${userId}::uuid)`; }
