...
test("production automation has no legacy rendering dependency",async()=>{const runner=await readFile(new URL("../scripts/run-daily-video-automation.mjs",import.meta.url),"utf8"),manifest=await readFile(new URL("../package.json",import.meta.url),"utf8");assert.match(runner,/pipeline:dry-run/);assert.match(runner,/youtube_publish/);assert.doesNotMatch(`${runner}\n${manifest}`,/creatomate/i);});
